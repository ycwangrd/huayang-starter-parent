...
/**
 * KingbaseES implementation of Schema.
 */
public class KingbaseESSchema extends Schema<KingbaseESDatabase, KingbaseESTable> {
    /**
     * Creates a new KingbaseES schema.
     *
     * @param jdbcTemplate The Jdbc Template for communicating with the DB.
     * @param database     The database-specific support.
     * @param name         The name of the schema.
     */
    protected KingbaseESSchema(JdbcTemplate jdbcTemplate, KingbaseESDatabase database, String name) {
    ...
    private List<String> generateDropStatementsForRoutines() throws SQLException {
        // #2193: KingbaseES 11 removed the 'proisagg' column and replaced it with 'prokind'.
        String isAggregate = database.getVersion().isAtLeast("11") ? "pg_proc.prokind = 'a'" : "pg_proc.proisagg";
        // PROCEDURE is only available from KingbaseES 11
    ...
    protected KingbaseESTable[] doAllTables() throws SQLException {
        ...
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new KingbaseESTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public Table getTable(String tableName) {
        return new KingbaseESTable(jdbcTemplate, database, this, tableName);
    }

    @Override
    protected Type getType(String typeName) {
        return new KingbaseESType(jdbcTemplate, database, this, typeName);
    }
}