...
public class KingbaseESConnection extends Connection<KingbaseESDatabase> {
    ...
    protected KingbaseESConnection(KingbaseESDatabase database, java.sql.Connection connection) {
    ...
    public Schema getSchema(String name) {
        return new KingbaseESSchema(jdbcTemplate, database, name);
    }

    @Override
    public <T> T lock(Table table, Callable<T> callable) {
        return new KingbaseESAdvisoryLockTemplate(jdbcTemplate, table.toString().hashCode()).execute(callable);
    }
}